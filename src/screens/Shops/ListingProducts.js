...
// Products of a "Listing" (a regular user's own shop) — matches web's
// Pages/MarketPlace/Product/AllProductofShop.jsx (a separate page + endpoint
// from the Marketplace browsing flow's AllProductofaShop.jsx).
const ListingProducts = ({ navigation, route }) => {
    ...
    const fetchProducts = async () => {
        try {
            const res = await apiGet(`${urls.getListingProducts}/${shopId}`);
            setProducts(res?.data || []);
        } catch (error) {
            console.log('ListingProducts fetchProducts error', error);
        } finally {
            setLoading(false);
            setRefreshing(false);
        }
    ...
    const deleteProduct = async (product) => {
        try {
            const res = await apiDelete(`${urls.deleteListingProduct}/${product?._id}`);
    ...
    const menuOptions = menuProduct
        ? [
              { label: 'Update Product', icon: 'edit-2', onPress: () => navigation.navigate('CreateProducts', { shopId, product: menuProduct }) },
    ...
    const styles = StyleSheet.create({
        ...
        headerSub: {
            ...
            color: isDarkMode ? '#9CA3AF' : '#6B7280', marginTop: 2,
        },
        addBtn: {
        ...
        priceBadge: {
            position: 'absolute', bottom: 8, left: 8, backgroundColor: 'rgba(0,0,0,0.65)',
            paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8,
        },
        priceBadgeText: { color: '#fff', fontSize: 11, fontFamily: FONTS_FAMILY.SourceSans3_Bold },
        cardBody: { padding: 10 },
        productName: { fontSize: 14, fontFamily: FONTS_FAMILY.SourceSans3_Medium },
        productDetails: {
            fontSize: 11, marginTop: 4, color: isDarkMode ? '#aaa' : '#7d7d7d',
        },
    ...
    const renderProduct = ({ item }) => (
        <TouchableOpacity
            activeOpacity={0.85}
            style={styles.card}
            onPress={() => navigation.navigate('ListingProductDetail', { productId: item?._id })}
        >
            <Image
            ...
            {!!item?.Price && (
                <View style={styles.priceBadge}>
                    <CustomText style={styles.priceBadgeText}>₹{Number(item.Price).toLocaleString()}</CustomText>
                </View>
            )}
            <TouchableOpacity
            ...
            <View style={styles.cardBody}>
                ...
                {!!item?.ProductDetails && (
                    <CustomText numberOfLines={2} style={styles.productDetails}>{item.ProductDetails}</CustomText>
                )}
            </View>
        </TouchableOpacity>
    );
        ...
        <View style={styles.container}>
            ...
            <SpaceBetweenRow style={styles.header}>
                <TouchableOpacity onPress={() => navigation.goBack()}>
                    <Icon name="arrow-left" size={22} color={isDarkMode ? white : '#111'} />
                </TouchableOpacity>
                <View style={{ flex: 1, marginLeft: 12 }}>
                    <CustomText style={styles.headerTitle}>{shopName || 'Products'}</CustomText>
                    <CustomText style={styles.headerSub}>{products?.length || 0} listed</CustomText>
                </View>
                <TouchableOpacity style={styles.addBtn} onPress={() => navigation.navigate('CreateProducts', { shopId })}>
                    ...
                    !loading && (
                        <View style={styles.emptyWrap}>
                            ...
                            <CustomText style={styles.emptySub}>
                                Tap the + button to add your first product to this listing.
                            </CustomText>
                        </View>
                    )
                }
