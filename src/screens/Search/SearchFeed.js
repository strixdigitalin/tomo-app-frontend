...
import { useSelector } from 'react-redux';
import Animated, {
  FadeInDown, FadeInUp, FadeIn, ZoomIn, SlideInRight,
...
const BrowseCategoryCard = React.memo(({ category, isDarkMode, onPress, index, primaryColor, secondaryColor }) => {
  ...
  return (
    ...
    >
      <GlowWrapper
        isDarkMode={isDarkMode} borderRadius={12}
        showStars={false} showShinePatches intensity="medium"
        containerStyle={{ width: '100%' }} glowColors={glowColors}
      >
        <TouchableOpacity
...
const UserCard = React.memo(({ item, isDarkMode, onPress, index, primaryColor, secondaryColor }) => {
  ...
  return (
    <Animated.View
      ...
      style={[S.userCardWrapper, cardStyle]}
    >
      <GlowWrapper
        isDarkMode={isDarkMode} borderRadius={12}
        showStars={false} showShinePatches intensity="low"
        containerStyle={{ flex: 1 }} glowColors={glowColors}
      >
        <TouchableOpacity
        ...
        >
          ...
          <Animated.View entering={FadeIn.duration(350).delay(Math.min(delay + 150, 350))} style={S.userCardText}>
...
const SearchScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    if (locationPermission) fetchNearbyUsers();
  }, [locationPermission, searchRadius]);
  ...
  const renderUserCard = useCallback(({ item, index }) => (
    <UserCard
      ...
      onPress={() => handleUserPress(item?._id)}
    />
  ), [isDarkMode, primaryColor, secondaryColor, handleUserPress]);
